...
class AgentManager:
    ...
    def download_agent(
        ...
        if version is None:
            ...
            params = {"author": author, "name": name, "version": version}
    ...
    def check_agent_updates(self, author: str, name: str, current_version: str) -> bool:
        response = requests.get(
            f"{self.base_url}/cerebrum/check_updates",
    ...
    def load_agent(
        ...
        if not local:
            ...
            if not agent_path.exists():
                print(
                    f"Agent {author}/{name} (v{version}) not found in cache. Downloading..."
                )
        ...
        spec = importlib.util.spec_from_file_location(
            module_name, str(temp_dir / entry_point)
        )
